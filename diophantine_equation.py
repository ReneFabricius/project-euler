...
from continued_fractions import (
    nthConvergentSQrootPrecomputed,
    continuedFracSQroot,
    periodicContinuedFraction,
    nConvergentsQuadraticSurdPrecomputed,
)
from euclidean_alg import ext_euclid
from primes import findDivisors, prime_fact_decomp
...
def pellEqu(D, n, maxPer=0):
    ...
    for k in range(2, n + 1):
        S += [
            (
                S[0][0] * S[k - 2][0] + D * S[0][1] * S[k - 2][1],
                S[0][0] * S[k - 2][1] + S[0][1] * S[k - 2][0],
            )
        ]
...
class ParametricSolution:
    ...
    def getSol(self, t):
        return (
            self.x0 + t * self.xq + t * t * self.xq2,
            self.y0 + t * self.yq + t * t * self.yq2,
        )
...
class ReccurentSolution:
    ...
    def nextSol(self):
        ...
        self.Xn, self.Yn = (
            self.P * self.Xn + self.Q * self.Yn + self.K,
            self.R * self.Xn + self.S * self.Yn + self.L,
        )
        return rX, rY
...
def linearDiophantineEq(D, E, F):
    if D == E == 0:
        if F == 0:
            return SolutionFlag.LIST_SOLUTIONS.value | SolutionFlag.NONE_IN_SOL.value, [
                (None, None)
            ]  # All x, y pairs are solution
        else:
    ...
    g, dc, ec = ext_euclid(D, E)
    if F % g != 0:
        return SolutionFlag.NO_SOLUTION, None

    d, e, f = D // g, E // g, F // g
    return SolutionFlag.PARAMETRIC_SOL, ParametricSolution(
        -f * dc, -f * ec, e, 0, -d, 0
    )
...
def findSolAmongConvergents(A, B, C, F, nd, per_q):
    ...
    if pl1 % 2 == 0:
        ...
        CVG1 = nConvergentsQuadraticSurdPrecomputed(
            CF1, pi1, pi1 + 2 * per_q * pl1, neg1
        )
    ...
    if pl2 % 2 == 0:
        ...
        CVG2 = nConvergentsQuadraticSurdPrecomputed(
            CF2, pi2, pi2 + 2 * per_q * pl2, neg2
        )
    ...
    for cvg in CVG:
        ...
        if (
            A * cvg.numerator**2
            + B * cvg.numerator * cvg.denominator
            + C * cvg.denominator**2
            + F
            == 0
        ):
            ...
            if gcd(A, gcd(B, dF)) == 1:
                ...
                for ps in range(abs(dF)):
                    if (A * ps * ps + B * ps + C) % dF == 0:
                        S += [ps]
                for s in S:
                    Sldvyzs = findSolAmongConvergents(
                        -(A * s * s + B * s + C) // dF,
                        2 * A * s + B,
                        -A * dF,
                        -1,
                ...
                for s in S:
                    Sldvxzs = findSolAmongConvergents(
                        -(C * s * s + B * s + A) // dF,
                        2 * C * s + B,
                        -C * dF,
                        -1,
                ...
                if i != 0 and m != 0:
                    ...
                    if lf == SolutionFlag.PARAMETRIC_SOL:
                        ...
                        c = A * j * j + B * j * n + C * n * n
                        hf, hS = homogenousHyperbolicDiophantineEqFewSols(
                            a, b, c, dF, nd, per_q
                        )
                        if hS:
                            for hs in hS:
...
def quadraticDiophantineEq(A, B, C, D, E, F):
    ...
    if B * B - 4 * A * C == 0:  # Parabolic case
        ...
        for u in U:
            s += [
                ParametricSolution(
                    -(sqr_c * g * u * u + E * u + sqr_c * F) // u_limiter,
                    (sqr_a * g * u * u + D * u + sqr_a * F) // u_limiter,
                    -(E + 2 * sqr_c * g * u),
                    sqr_c * g * (-u_limiter),
                    (D + 2 * sqr_a * g * u),
                    sqr_a * g * u_limiter,
...
def findOneSolGeneralHyperbolic(A, B, C, D, E, F, m):
    ...
    c = g
    if (
        4 * A * (4 * A * C * F - A * E * E - B * B * F + B * D * E - C * D * D)
    ) % g != 0:
        print(
            str(g)
            + " does not divide "
            + str(4 * A * C * F - A * E * E - B * B * F + B * D * E - C * D * D)
        )
...
def tryComputeKL(A, B, C, D, E, F, r, s):
    ...
    R = A * s
    S = r + B * s
    if (C * D * (P + S - 2) + E * (B - B * r - 2 * A * C * s)) % (
        4 * A * C - B * B
    ) == 0:
        K = (C * D * (P + S - 2) + E * (B - B * r - 2 * A * C * s)) // (
            4 * A * C - B * B
        )
        if (D * (B - B * r - 2 * A * C * s) + A * E * (P + S - 2)) % (
            4 * A * C - B * B
        ) == 0:
            L = (D * (B - B * r - 2 * A * C * s) + A * E * (P + S - 2)) // (
                4 * A * C - B * B
            ) + D * s
            return [(P, Q, K, R, S, L)]
...
def testGeneralRecc(A, B, C, D, E, F):
    ...
    for rec in Rc:
        ...
        for ss in SS:
            ...
            while (
                Xnu % de == Ynu % de == 0
                and abs(Xnu // de) < abs(X)
                and abs(Ynu // de) < abs(Y)
            ):
                X, Y = Xnu // de, Ynu // de
                Xnu = Q * (Y - L) - S * (X - K)
                Ynu = R * (X - K) - P * (Y - L)

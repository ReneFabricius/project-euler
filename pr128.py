    ...
    if one_prev:
        ...
        if mod == c - 1:
            ...
            if next_dir_n == 0:
                ...
                NGBRS += [
                    prev_beg + next_dir_n * (c - 1),
                    prev_beg + next_dir_n * (c - 1) - 1,
                ]
        else:
            NGBRS += [
                prev_beg + (c - 1) * last_dir_n + mod - 1,
                prev_beg + (c - 1) * last_dir_n + mod,
            ]

        # 2 fol

        NGBRS += [
            next_beg + (c + 1) * last_dir_n + mod,
            next_beg + (c + 1) * last_dir_n + mod + 1,
        ]

...
def problem137(l):
    ...
    while len(L) < l:
        ...
        LinS = []
        f, sol = quadraticDiophantineEq(
            0, 0, 0, abs(RS[min_k]) - min_k - 1, 2 * min_k, 0
        )
        LinS += [tuple([f, sol])]
        f, sol = quadraticDiophantineEq(
            0, 0, 0, -abs(RS[min_k]) - min_k - 1, 2 * min_k, 0
        ...
        for linS in LinS:
            if linS[0] == SolutionFlag.PARAMETRIC_SOL:
                if sign(linS[1].getCoefs()[0]) == sign(linS[1].getCoefs()[2]) and abs(
                    linS[1].getCoefs()[0]
                ) < abs(linS[1].getCoefs()[2]):
                    L += [
                        tuple(
                            [
                                min_k,
                                abs(linS[1].getCoefs()[0]),
                                abs(linS[1].getCoefs()[2]),
                            ]
                        )
                    ]

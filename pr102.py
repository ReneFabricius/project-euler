...
def problem102():
    ...
    for t in f:
        ...
        T = [int(x) for x in T]
        T = [
            numpy.matrix([[T[0]], [T[1]], [1]]),
            numpy.matrix([[T[2]], [T[3]], [1]]),
            numpy.matrix([[T[4]], [T[5]], [1]]),
        ]
        ...
        if not (Ot[1] == 0 and Ot[0] >= 0):
            ...
            Rt = numpy.matrix(
                [[cos(ra), -sin(ra), 0], [sin(ra), cos(ra), 0], [0, 0, 1]]
            )
            Tt = [Rt * p for p in Tt]
            Ot = Rt * Ot
...
def problem102cross():
    ...
    for t in f:
        ...
        T = [int(x) for x in T]
        T = [
            numpy.array([T[0], T[1], 0]),
            numpy.array([T[2], T[3], 0]),
            numpy.array([T[4], T[5], 0]),
        ]
        c1 = numpy.cross(T[1] - T[0], -T[0])
        c2 = numpy.cross(T[2] - T[1], -T[1])
        c3 = numpy.cross(T[0] - T[2], -T[2])
        if (c1[2] > 0 and c2[2] > 0 and c3[2] > 0) or (
            c1[2] < 0 and c2[2] < 0 and c3[2] < 0
        ):
            count += 1

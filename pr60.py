...
def problem60(l, n):
    ...
    PG = set(P)
    PC = [
        set() for i in range(l + 1)
    ]  # na i-tej pozicii - mnozina prvocisiel konkatovatelnych s i
    PN = [
        [] for j in range(n + 1)
    ]  # na indexoch 2-n vsetky mozne tuple s pozadovanymi vlastnostami dlzky index
    ...
    for m in range(2, n + 1):
        ...
        if mN[m - 1] + l + 1 < mN[m]:
            print(
                "Mozna existencia "
                + str(m)
                + " - prvkoveho tuple prvocisel s mensim suctom ako minimalny najdeny, potrebne spustit problem40 pre l aspon "
                + str(mN[m] - mN[m - 1] - 1)
            )

...
def prime_fact_decomp(n):
    "Find prime decomposition, adds a factor 0 if possibly the decomposition is incomplete"
    INIT_P_LIMIT = 10**4
    P_LIMIT = 10**7
    F = Counter()
    if n == 1:
...
def find_divisors_range(l):
    ...
    def _compute_divisors(n):
        ...
        while _incr():
            divs.append(pw_prod([pris[i] ** cur_pows[i] for i in range(len(cur_pows))]))
...
def find_divisors_range_simple(l):
    ...
    for d in range(2, l):
        if (
            d < 100
            or (d < 1000 and d % 100 == 0)
            or (d < 10000 and d % 1000 == 0)
            or (d < 100000 and d % 10000 == 0)
            or d % 100000 == 0
        ):

    ...
    if (potential_addition[1] & must_be_incl) == must_be_incl:
        for pot_add in potential_addition[1] - must_be_incl:
            if (
                len(
                    set(
                        [pot_add_p ** A[pot_add][pot_add_p] for pot_add_p in A[pot_add]]
                    )
                    & resolved
                )
                > 0
        ...
        if len(unresolved) == 0:
            ...
            if sm + mpq(1, 4) < mpq(1, 2):
                base_adepts = [
                    adpt
                    for adpt in G
                    if 0 in G[adpt] and adpt not in ignored and adpt not in resolved
                ]
            ...
            if 0 in G[unres_M]:
                for potential_addition in G[unres_M][0]:
                    addible = testAddibility(
                        potential_addition, must_be_incl, A, resolved
                    )

...
def problem51(l):
    ...
    while True:
        ...
        for p in P:
            d_l = (
                10 - l
            )  # najvyssia cifra ktoru mozem zamienat, aby som mohol ziskat pozadovany pocet prvocisiel
            D = [
                [] for i in range(1, d_l + 2)
            ]  # na i-tej pozicii: zoznam pozicii v cisle zprava od nuly na ktorych sa nachadza cifra i
                ...
                for c_l in range(1, len(D[d]) + 1):  # cez mozne dlzky ich kombinacii
                    for c in combinations(D[d], c_l):  # cez kombinacie danej dlzky
                        ...
                        for d_e in range(d + 1, 10):  # cez moznych kandidatov na vymenu
                            pX = exchangeDigits(
                                p, d_e, c
                            )  # cislo s vymenenymi pozadovanymi ciframi
                            if pX in PS:
                                count += 1
                                if count == l:
                                    print(
